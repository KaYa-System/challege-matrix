...
export function ParticipantDashboard() {
  ...
  async function loadDashboardData() {
    try {
      ...
      // Vérifier si l'utilisateur a accepté les termes et récupérer son niveau
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) throw new Error('Utilisateur non connecté');

      const { data: userData, error: userError } = await supabase
          .from('users')
          .select('terms_accepted, current_level') // Ajout de current_level
          .eq('id', user.id)
          .single();
      ...
      // Charger le challenge correspondant au niveau de l'utilisateur
      const { data: challengeData, error: challengeError } = await supabase
          .from('challenges')
          .select('*')
          .eq('status', 'active')
          .eq('level', userData.current_level) // Utiliser le niveau de l'utilisateur
          .single();
      ...
      if (challengeData) {
        ...
        // Charger le prochain challenge si le challenge actuel est complété
        if (participationData?.status === 'completed') {
          const nextLevel = userData.current_level + 1; // Utiliser le niveau de l'utilisateur
          console.log('Recherche du niveau suivant:', nextLevel);
          ...
          if (nextChallengeError) {
            ...
            console.log('Prochain challenge trouvé:', nextChallengeData);
  ...
  async function handleNextChallenge() {
    ...
    try {
      console.log('Transition vers le niveau suivant...');
      console.log('Challenge actuel:', activeChallenge.level);
      console.log('Prochain challenge:', nextChallenge.level);
      ...
      // Mettre à jour le niveau de l'utilisateur et les termes
  ...
  // Si aucun challenge n'est actif mais qu'il y a un prochain challenge disponible
  ...
  const submitButtonDisabled = showNextChallengeButton || new Date() < nextSubmissionDate;
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-gradient-to-br from-emerald-50 to-teal-50 rounded-lg p-4">
            ...
            <div className="mt-2">
              <p className="text-xs text-gray-600">Prochaine soumission dans:</p>
              <Countdown
                date={nextSubmissionDate}
            ...
            <div className="mt-2 space-y-2">
              {showNextChallengeButton ? (
                  ...
                  <button
                      ...
                      title={new Date() < nextSubmissionDate ? "Vous devez attendre l'heure de soumission" : ""}
                  >
